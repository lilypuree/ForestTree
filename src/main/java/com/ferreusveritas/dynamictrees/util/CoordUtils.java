...
public class CoordUtils {
	...
	public static boolean isSurroundedByLoadedChunks(World world, BlockPos pos) {
		for(Surround surr: CoordUtils.Surround.values()) {
			...
			if(world.getChunkProvider().getChunk((pos.getX() >> 4) + dir.getX(), (pos.getZ() >> 4) + dir.getZ(), false) == null){
				return false;
			}
		}
		return true;
...
//
	...
	public static BlockPos findGround(World world, BlockPos startPos) {
		...
		while(world.getBlockState(pos).isOpaqueCube(world, pos)) {
			pos.setPos(pos.getX(), pos.getY() + 1, pos.getZ());
		}
		//Dive down until we are again
		while(!world.getBlockState(pos).isOpaqueCube(world, pos) && pos.getY() > 50) {
			pos.setPos(pos.getX(), pos.getY() - 1, pos.getZ());
		}
		return pos;
	}
...
//						while(true) {
//							if(currentDir < Direction.Plane.HORIZONTAL.) {
