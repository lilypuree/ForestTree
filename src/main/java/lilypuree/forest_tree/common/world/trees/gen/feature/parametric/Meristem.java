...
public abstract class Meristem {
    ...
    protected int order;
    //TODO
    //actual node length calculation
    ...
    public void init(int age, int order, BlockPos pos, GrowthVec dir, MeristemFactory factory, MeristemGrower grower, MeristemTerminator killer) {
        this.age = age;
        this.order = order;
    ...
    public Collection<Meristem> generateAxillaries(Random rand) {
        ...
        if (grower.shouldGenerateNode(interval, isTerminal()) || !this.isAlive() && isTerminal()) {
            ...
            if (isTerminal() && rand.nextFloat() < terminator.getBranchDeathRate(newAge)) {
                return newMeristems;
            } else if (alive) {
